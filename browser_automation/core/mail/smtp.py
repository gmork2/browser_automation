...
from core.mail.utils import sanitize_address, DNS_NAME
...
class EmailBackend(object):
    ...
    def __init__(self, host=None, port=None, username=None, password=None,
        ...
        self.ssl_keyfile = config['email_ssl_keyfile'] if ssl_keyfile is None else ssl_keyfile
        self.ssl_certfile = config['email_ssl_certfile'] if ssl_certfile is None else ssl_certfile

        if self.use_ssl and self.use_tls:
            raise ValueError(
                "EMAIL_USE_TLS/EMAIL_USE_SSL are mutually exclusive, so only set "
                "one of those conf to True.")
    ...
    def _send(self, email_message):
        ...
        if not email_message.recipients():
            return False
        encoding = email_message.encoding or config['default_charset']
